...
(deftest test-abs
  (testing "Absolute value (magnitude) calculation"
    (is (= 5.0 (abs [3 4])))                    ; 3-4-5 triangle
    (is (= 13.0 (abs [5 12])))                  ; 5-12-13 triangle
    (is (= 1.0 (abs [1 0])))                    ; Real number
    (is (= 1.0 (abs [0 1])))                    ; Pure imaginary
    (is (= 0.0 (abs [0 0])))                    ; Zero
    (is (= 5.0 (abs [-3 -4])))                  ; Negative components
    (is (approximately-equal? 2.236 (abs [1 2]) 0.001))))  ; √5 ≈ 2.236
